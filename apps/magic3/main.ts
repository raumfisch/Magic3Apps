...
interface WOS {
  ...
  ticker: any;
  settings: any;
  buzz(v: number): Promise<void>;
  batV(): number;
  isCharging(): boolean;
  setLCDTimeout(v: number): void;
  setLCDBrightness(v: number): void;
  init(): void;
  sleep(): void;
  bright(): void;
  wake(): void;
  setLCDPower(b: boolean): void;
  isLCDOn(): boolean;
  tick(): void;
}

interface Bangle extends WOS {
  POWER: boolean;
}
...
const wOS: WOS = {
  ...
  ticker: undefined,
  settings: undefined,
  buzz: function (v: number): Promise<void> {
    return new Promise(function (resolve, reject) {
      v = v ? v : 100;
      if (!wOS.VIBRATE) resolve();
      else {
        analogWrite(wOS.BUZZPIN, 0.75, 20000);
        setTimeout(() => {
          wOS.BUZZPIN.set();
          resolve();
        }, v);
  ...
  init: () => {
    var s = STOR.readJSON("settings.json", 1) || {
      ontime: 10,
      bright: 0.5,
      timezone: 1,
      faceup: true,
  ...
  bright: () => {
    ...
    var b =
      hrs >= ds && hrs < ls
        ? wOS.BRIGHT
        : hrs >= ls && hrs < ns
        ? wOS.settings.lowbright
        : wOS.settings.nightbright;
    wOS.brightness(b);
  },
  wake: () => {
    wOS.awake = true;
...
function watchBat() {
  setWatch(
    ...
    wOS.BATPIN,
    { edge: "both", repeat: true, debounce: 500 }
  );
}
wOS.init();
eval(STOR.read(process.env.BOARD == "P8" ? "lcd-p8.js" : "lcd.js"));
const g = ST7789();
g.theme = wOS.settings.theme
  ? wOS.settings.theme
  : {
      fg: 0xffff,
      bg: 0,
...
if (process.env.BOARD == "ROCK") eval(STOR.read("cst816s.js"));
else if (process.env.BOARD == "P8") eval(STOR.read("cst716-p8.js"));
else eval(STOR.read("cst716.js"));
TC.start();
//console.log("loaded touch");
if (wOS.STEPS) eval(STOR.read("accel_step.js"));
else eval(STOR.read("accel.js"));
ACCEL.init();
ACCEL.on("faceup", () => {
  if (!wOS.awake) wOS.wake();
});
...
if (STOR.read("android.boot.js")) {
  Bangle.load = function (filename) {
    eval(STOR.read(filename));
  };
